...
function normalizeMemoryPayload(payload) {
  ...
  async function streamExecution(data = {}, handlers = {}) {
    const response = await fetch(ROUTES.streamExecution, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Accept: "text/event-stream, application/json, text/plain",
      ...
      body: JSON.stringify({
        ...
        execution_state:
          isObject(data.execution_state)
            ? data.execution_state
            : isObject(data.executionState)
              ? data.executionState
              : isObject(data.execution)
                ? data.execution
                : undefined,
      }),
      signal: handlers.signal,
    ...
    if (!response.ok) {
      ...
      throw buildError(
        payloadMessage ||
          `POST failed: ${ROUTES.streamExecution}`,
        {
          status: response.status,
          statusText: response.statusText,
          payload,
    ...
    while (true) {
      ...
      for (const part of parts) {
        ...
        if (
          eventName === "start" &&
          typeof handlers.onStart === "function"
        ) {
          handlers.onStart(
            eventData,
            parsed
        ...
        if (
          ...
          typeof handlers.onStepStart === "function"
        ) {
        ...
        if (
          eventName === "step_done" &&
          typeof handlers.onStepDone === "function"
        ) {
          handlers.onStepDone(
            eventData,
            parsed
        ...
        if (
          eventName === "done" &&
          typeof handlers.onDone === "function"
        ) {
          handlers.onDone(
            eventData,
            parsed
  ...
  api.streamExecution = streamExecution;
