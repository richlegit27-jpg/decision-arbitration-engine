...
def api_memory_add() -> Any:
    try:
        data = request.get_json(silent=True) or {}
        text = normalize_text(data.get("text") or "").strip()
        kind = normalize_text(data.get("kind") or "note").strip() or "note"
        ...
        return jsonify({
            "ok": False,
            "error": str(exc) or "memory add failed",
        }), 500
...
def api_memory_delete() -> Any:
    try:
        data = request.get_json(silent=True) or {}
        memory_id = normalize_text(data.get("id") or "").strip()
        ...
        next_items = [
            ...
            if normalize_text((item or {}).get("id") or "").strip() != memory_id
        ]
        ...
        return jsonify({
            "ok": False,
            "error": str(exc) or "memory delete failed",
        }), 500
...
def _looks_like_bare_url_only(text: str) -> bool:
    raw = str(text or "").strip()
    if not raw:
        return False

    # only allow a single bare URL as the whole message
    if " " in raw or "\n" in raw or "\t" in raw:
        return False

    candidate = _normalize_candidate_url(raw)
...
def _extract_web_autoroute_url(user_text: str) -> str:
    raw = str(user_text or "").strip()
    if not raw:
        return ""

    # explicit command always wins
    if raw.lower().startswith("/web "):
        explicit = _normalize_candidate_url(raw[5:].strip())
        parsed = urlparse(explicit)
        if parsed.scheme in ("http", "https") and parsed.netloc and "." in parsed.netloc:
            return explicit
        return ""

    # do NOT autoroute sentences, markdown links, code, or mixed text
    if "[" in raw or "](" in raw or "`" in raw:
        return ""

    if _looks_like_bare_url_only(raw):
...
autoroute_url = _extract_web_autoroute_url(user_text)

if autoroute_url:
    return handle_web_fetch_for_chat(
        url=autoroute_url,
        payload=payload,
        user_text=user_text,
    )
