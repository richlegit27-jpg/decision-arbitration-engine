...
items.slice(0, 50).forEach(function (item) {
    ...
    row.style.cssText =
        "padding:10px 12px;border:1px solid rgba(255,255,255,.12);border-radius:12px;margin-bottom:10px;white-space:pre-wrap;word-break:break-word;";

    var text = item.text || item.content || item.memory || item.value || JSON.stringify(item, null, 2);
    var id = item.id || item.memory_id || item.uuid || "";

    row.innerHTML = `
        <div style="font-size:11px;opacity:.7;margin-bottom:6px;text-transform:uppercase;">
            ${item.kind || item.type || item.category || "memory"}
        </div>
    ...
    var deleteBtn = row.querySelector(".nova-memory-delete-btn");
...
window.NovaOpenDesktopMemoryLegacy = loadDesktopMemoryLegacy;
window.NovaDesktopOpenMemoryRescueLegacy = loadDesktopMemoryLegacy;
window.NovaLoadMemoryLegacy = loadDesktopMemoryLegacy;
window.loadMemoryLegacy = loadDesktopMemoryLegacy;
window.loadMemoriesLegacy = loadDesktopMemoryLegacy;
