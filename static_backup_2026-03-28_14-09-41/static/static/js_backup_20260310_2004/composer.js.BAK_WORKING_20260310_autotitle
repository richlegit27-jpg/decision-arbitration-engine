...
function updateComposerState(){
  ...
  const hasText = !!text
  const hasPending = ensurePendingAttachments().length > 0
...
function safeAutoTitle(chatId, text){
  ...
  try{
    ...
    console.error("NovaComposer renderAll skipped:", error)
  }
}
...
async function sendCurrentMessage(options = {}){
  ...
  try{
    ...
    let chatId = String(state.activeChatId || "").trim()
    ...
    if(streamService && typeof streamService.send === "function"){
      ...
      if(reply?.chat_id){
        state.activeChatId = String(reply.chat_id)
      }else if(reply?.chat?.chat_id || reply?.chat?.id){
        state.activeChatId = String(reply.chat.chat_id || reply.chat.id)
      }
      ...
      if(Array.isArray(reply?.messages) && reply.messages.length){
        ...
        appendAssistantMessage(String(reply.response || ""))
      }
    }

    safeAutoTitle(state.activeChatId || chatId, text)
