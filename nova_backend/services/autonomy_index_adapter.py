...
def extract_autonomy_index_input(user_text: str) -> Optional[str]:
    raw = str(user_text or "").strip()
    lowered = raw.lower()

    for prefix in AUTONOMY_INDEX_PREFIXES:
        if lowered == prefix:
            return ""
...
def build_autonomy_index_response(
    payload: Dict[str, Any],
    session_service: Any = None,
) -> Optional[Dict[str, Any]]:
    data = payload if isinstance(payload, dict) else {}
    user_text = _payload_user_text(data)
    index_input = extract_autonomy_index_input(user_text)
    ...
    return {
        ...
        "debug": {
            "route": "autonomy_index_command",
            "mode": "autonomy_ladder_index_only",
            "adapter": "autonomy_index_adapter",
        },
    }
