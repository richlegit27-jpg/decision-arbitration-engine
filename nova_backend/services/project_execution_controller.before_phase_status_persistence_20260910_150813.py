...
class ProjectExecutionController:
    ...
    def _persist_phase_statuses(
        self,
        project,
        tasks,
        active_phase_id=None,
        blocked_phase_id=None,
    ):
        """
        Persist derived phase statuses without changing task execution.

        Status rules:
        - active: current phase with runnable work
        - blocked: current phase has failed/blocked work and no runnable work
        - completed: every assigned task has a successful terminal status
        - planned: phase has not started or has no active work
        """
        if not isinstance(project, dict):
            return
        ...
        for phase in phases:
            ...
            if not phase_tasks:
                next_status = "planned"
            elif self._phase_is_successfully_complete(
                phase_tasks
            ):
