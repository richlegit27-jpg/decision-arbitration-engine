...
def create_autonomy_guard_cleanup_review(app_path: str = "app.py") -> Dict[str, object]:
    ...
    return {
        ...
        "safe_migration_plan": [
            ...
            "Confirm adapter-owned guards are installed and pass before touching old fallback guards.",
            "Identify only old fallback command guards that are now shadowed by adapter-owned guards.",
            "Prove every adapter route keeps the expected route, mode, and debug fields.",
            "Do not remove any old fallback guard in this review-only step.",
            "If cleanup is approved later, remove at most one old fallback guard per commit after clean smokes.",
        ],
