...
# NOVA_ATTACHMENT_ANALYSIS_BRIDGE_DOCX_SAFE_20260608
def _nova_extract_docx_text_20260608(file_path):
    import zipfile
    import xml.etree.ElementTree as ET
...
def _nova_read_attachment_text_20260608(file_path):
    file_path = Path(file_path)
    ...
    if suffix == ".docx":
        return _nova_extract_docx_text_20260608(file_path)

    if suffix in {
        ".txt", ".md", ".markdown", ".py", ".js", ".css", ".html", ".htm",
        ".json", ".csv", ".xml", ".log", ".yml", ".yaml"
    }:
        try:
        ...
        # Avoid summarizing binary/zip/docx garbage as text.
        if cleaned.startswith("PK\x03\x04") or "[Content_Types].xml" in cleaned[:500]:
            return ""
...
def _nova_resolve_attachment_path_20260608(attachment):
    ...
    for key in ("url", "file_url"):
        value = attachment.get(key)
        if value:
            name = str(value).split("?")[0].rstrip("/").split("/")[-1]
            if name:
                candidates.append(uploads / name)
...
def _nova_execute_attachment_analysis_20260608(self, attachments=None, *args, **kwargs):
    attachments = attachments or []
    ...
    if not isinstance(attachments, list):
        return "No usable attachment data was provided."

    summaries = []

    for attachment in attachments:
        file_path = _nova_resolve_attachment_path_20260608(attachment)
        ...
        if not file_path:
            summaries.append(f"Attachment {name}: file was not found in uploads.")
            continue

        suffix = file_path.suffix.lower()
        text = _nova_read_attachment_text_20260608(file_path)
        ...
        if suffix in {".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"}:
            summaries.append(
                f"Attachment {name}: image file received. "
                "Image analysis should be handled by the vision/image pipeline."
            )
            continue

        if suffix == ".docx":
            summaries.append(
                f"Attachment {name}: DOCX file detected, but no readable Word text was extracted."
            )
            continue

        summaries.append(
            f"Attachment {name}: unsupported or binary file type. "
            "Nova will not summarize raw binary bytes."
        )

    return "\n\n".join(summaries).strip() or "No readable attachment content was found."


try:
    ChatService._execute_attachment_analysis = _nova_execute_attachment_analysis_20260608
except NameError:
    pass

# NOVA_ATTACHMENT_STRING_SAFE_OVERRIDE_20260608
def _nova_normalize_attachment_20260608(attachment):
    """
    Accept dict attachments, filename strings, URL strings, and path strings.
    Always return a dict so downstream code can safely use .get().
    """
    if isinstance(attachment, dict):
        return attachment
    ...
    if isinstance(attachment, str):
        ...
        if value.startswith("/api/uploads/"):
            return {
                ...
                "original_filename": value.rstrip("/").split("/")[-1],
            }
...
def _nova_resolve_attachment_path_string_safe_20260608(attachment):
    from pathlib import Path

    root = Path(r"C:\Users\Owner\nova")
    uploads = root / "uploads"
    attachment = _nova_normalize_attachment_20260608(attachment)
...
def _nova_extract_docx_text_string_safe_20260608(file_path):
    from pathlib import Path
    import zipfile
    import xml.etree.ElementTree as ET
...
def _nova_read_attachment_text_string_safe_20260608(file_path):
    from pathlib import Path
    ...
    if suffix == ".docx":
        return _nova_extract_docx_text_string_safe_20260608(file_path)
...
def _nova_execute_attachment_analysis_string_safe_20260608(self, attachments=None, *args, **kwargs):
    if attachments is None:
        attachments = []

    if isinstance(attachments, (dict, str)):
        attachments = [attachments]
    ...
    for raw_attachment in attachments:
        attachment = _nova_normalize_attachment_20260608(raw_attachment)

        name = (
            attachment.get("original_filename")
            or attachment.get("filename")
            or attachment.get("name")
            or str(raw_attachment)
            or "attachment"
        )

        file_path = _nova_resolve_attachment_path_string_safe_20260608(attachment)
        ...
        suffix = file_path.suffix.lower()
        text = _nova_read_attachment_text_string_safe_20260608(file_path)
...
# NOVA_FINAL_ATTACHMENT_ROUTE_BEATS_WEB_LOCK_V2_20260624
# Final guard: attachment-injected text must beat every web/news route wrapper.
# V2 keeps fallback compatible with older _decide_route signatures.
try:
    _NOVA_PRE_ATTACHMENT_ROUTE_DECIDE_V2_20260624 = ChatService._decide_route

    def _nova_call_previous_decide_route_v2_20260624(
        self,
        user_text="",
        attachments=None,
        memory_context="",
        ...
        **kwargs,
        ...
        try:
            return _NOVA_PRE_ATTACHMENT_ROUTE_DECIDE_V2_20260624(
                self,
                user_text=user_text,
                attachments=attachments,
                memory_context=memory_context,
    ...
    def _nova_attachment_route_beats_web_decide_v2_20260624(
        self,
        user_text="",
        attachments=None,
        memory_context="",
        ...
        **kwargs,
        ...
        if any(marker in lower for marker in markers):
            return {
                "route": getattr(self, "ROUTE_ATTACHMENT_ANALYSIS", "attachment_analysis"),
                "mode": "attachment_text_injected_final_lock_v2",
                "confidence": 0.995,
                "reasons": ["final_attachment_context_blocks_web_route_v2"],
                "save_artifact": False,
                "save_memory": True,
                "use_memory": True,
                "blocked": ["web_fetch"],
        ...
        if attachments and any(
            ...
            return {
                "route": getattr(self, "ROUTE_ATTACHMENT_ANALYSIS", "attachment_analysis"),
                "mode": "real_attachment_final_lock_v2",
                "confidence": 0.99,
                "reasons": ["final_real_attachment_blocks_web_route_v2"],
                "save_artifact": False,
                "save_memory": True,
                "use_memory": True,
                "blocked": ["web_fetch"],
        ...
        return _nova_call_previous_decide_route_v2_20260624(
            self,
            user_text=user_text,
            attachments=attachments,
            memory_context=memory_context,
...
# NOVA_PROJECT_STATE_RECALL_20260630
# Narrow project-state recall layer.
# Answers project checkpoint questions from data/nova_project_state.json + live git state.
# Does not override active execution missions: "next" / "k" still pass through when execution is active.
try:
    from nova_backend.services.project_state_service import answer_project_state_question
    ...
    if not getattr(ChatService, "_NOVA_PROJECT_STATE_RECALL_20260630", False):
        ...
        def _nova_project_state_runtime_state_20260630(self):
            candidates = [
                getattr(self, "execution_state", None),
                getattr(self, "_execution_state", None),
                getattr(self, "planner_state", None),
                getattr(getattr(self, "planner_service", None), "execution_state", None),
                getattr(getattr(self, "execution_service", None), "state", None),
                getattr(getattr(self, "session_service", None), "execution_state", None),
            ]
        ...
        def _nova_project_state_response_obj_20260630(reply):
            return {
                "ok": True,
                "success": True,
                "content": reply,
        ...
        def _nova_project_state_recall_handle_20260630(self, *args, **kwargs):
            try:
                ...
                if reply:
                    return _nova_project_state_response_obj_20260630(reply)
            except Exception as _nova_project_state_recall_error_20260630:
                try:
                    print(
                        "[NOVA_PROJECT_STATE_RECALL_20260630] bypass:",
                        _nova_project_state_recall_error_20260630,
                    )
                except Exception:
                    pass

            return _NOVA_PRE_PROJECT_STATE_RECALL_HANDLE_20260630(self, *args, **kwargs)
...
# NOVA_PROJECT_STATE_RECALL_FINAL_20260630
# Final project-state recall owner.
# Loads project_state_service.py by file path to avoid service-package circular imports.
# Does not override active execution missions: "next" / "k" still pass through when execution is active.
try:
    import importlib.util as _nova_project_state_importlib_util_20260630
    from pathlib import Path as _NovaProjectStatePath20260630
    ...
    if not getattr(ChatService, "_NOVA_PROJECT_STATE_RECALL_FINAL_20260630", False):
        ...
        def _nova_project_state_answer_final_20260630(user_text, runtime_execution_state=None):
            service_path = (
                _NovaProjectStatePath20260630(__file__)
                .resolve()
                .with_name("project_state_service.py")
            )

            spec = _nova_project_state_importlib_util_20260630.spec_from_file_location(
                "_nova_project_state_service_direct_20260630",
                str(service_path),
            )
            ...
            module = _nova_project_state_importlib_util_20260630.module_from_spec(spec)
        ...
        def _nova_project_state_response_obj_final_20260630(reply):
            return {
                "ok": True,
                "success": True,
                "content": reply,
        ...
        def _nova_project_state_recall_final_handle_20260630(self, *args, **kwargs):
            try:
                ...
                try:
                    print(
                        "[NOVA_PROJECT_STATE_RECALL_FINAL_20260630] bypass:",
                        _nova_project_state_recall_final_error_20260630,
                    )
                except Exception:
                    pass
    ...
    try:
        print(
            "[NOVA_PROJECT_STATE_RECALL_FINAL_20260630] failed:",
            _nova_project_state_final_install_error_20260630,
        )
    except Exception:
        pass


# NOVA_PROJECT_STATE_IDLE_NEXT_FALLBACK_REPAIR_20260630
# If idle execution fallback answers "No active execution mission" for next/k,
# convert it to project-state recall. Active execution still wins because it will
# not return the idle fallback text.
try:
    import importlib.util as _nova_project_state_idle_importlib_util_20260630
    from pathlib import Path as _NovaProjectStateIdlePath20260630
    ...
    if not getattr(ChatService, "_NOVA_PROJECT_STATE_IDLE_NEXT_FALLBACK_REPAIR_20260630", False):
        ...
        def _nova_project_state_idle_set_content_20260630(payload, reply):
            if isinstance(payload, dict):
        ...
        def _nova_project_state_idle_answer_20260630(user_text):
            service_path = (
                _NovaProjectStateIdlePath20260630(__file__)
                .resolve()
                .with_name("project_state_service.py")
            )

            spec = _nova_project_state_idle_importlib_util_20260630.spec_from_file_location(
                "_nova_project_state_service_idle_direct_20260630",
                str(service_path),
            )
            ...
            module = _nova_project_state_idle_importlib_util_20260630.module_from_spec(spec)
            spec.loader.exec_module(module)
            ...
            # Important: pass no runtime execution state here.
            # We are only here after the old handler already said execution is idle.
            return answer_fn(user_text, runtime_execution_state=None)

        def _nova_project_state_idle_next_handle_20260630(self, *args, **kwargs):
            result = _NOVA_PRE_PROJECT_STATE_IDLE_NEXT_HANDLE_20260630(self, *args, **kwargs)

            try:
                user_text = _nova_project_state_idle_extract_text_20260630(args, kwargs).strip().lower()
                content = _nova_project_state_idle_get_content_20260630(result)

                is_idle_next = user_text in {"next", "k", "ok", "okay", "continue"}
                is_idle_execution_fallback = "no active execution mission" in str(content or "").lower()

                if is_idle_next and is_idle_execution_fallback:
                    reply = _nova_project_state_idle_answer_20260630(user_text)
                    if reply:
                        return _nova_project_state_idle_set_content_20260630(result, reply)
            except Exception as _nova_project_state_idle_next_error_20260630:
                try:
                    print(
                        "[NOVA_PROJECT_STATE_IDLE_NEXT_FALLBACK_REPAIR_20260630] bypass:",
                        _nova_project_state_idle_next_error_20260630,
                    )
                except Exception:
                    pass
    ...
    try:
        print(
            "[NOVA_PROJECT_STATE_IDLE_NEXT_FALLBACK_REPAIR_20260630] failed:",
            _nova_project_state_idle_next_install_error_20260630,
        )
    except Exception:
        pass


# NOVA_PROJECT_STATE_IDLE_NEXT_RESPONSE_REPAIR_FINAL_20260630
# Response-aware final repair for idle next/k.
# Converts the old idle execution fallback into project-state recall even when
# ChatService.handle returns a Flask Response object instead of a dict.
try:
    import json as _nova_project_state_idle_json_final_20260630
    import importlib.util as _nova_project_state_idle_response_importlib_util_20260630
    from pathlib import Path as _NovaProjectStateIdleResponsePath20260630
    ...
    if not getattr(ChatService, "_NOVA_PROJECT_STATE_IDLE_NEXT_RESPONSE_REPAIR_FINAL_20260630", False):
        ...
        def _nova_project_state_idle_response_load_answer_20260630(user_text):
            service_path = (
                _NovaProjectStateIdleResponsePath20260630(__file__)
                .resolve()
                .with_name("project_state_service.py")
            )

            spec = _nova_project_state_idle_response_importlib_util_20260630.spec_from_file_location(
                "_nova_project_state_service_response_direct_20260630",
                str(service_path),
            )
            ...
            module = _nova_project_state_idle_response_importlib_util_20260630.module_from_spec(spec)
            spec.loader.exec_module(module)
        ...
        def _nova_project_state_idle_response_content_from_dict_20260630(payload):
            assistant = payload.get("assistant_message")
            if isinstance(assistant, dict):
                content = assistant.get("content")
                if isinstance(content, str):
        ...
        def _nova_project_state_idle_response_patch_result_20260630(result, reply):
            if isinstance(result, dict):
                return _nova_project_state_idle_response_patch_dict_20260630(result, reply)
            ...
            if hasattr(result, "get_data") and hasattr(result, "set_data"):
                raw = result.get_data(as_text=True)
                data = _nova_project_state_idle_json_final_20260630.loads(raw)
                if isinstance(data, dict):
                    data = _nova_project_state_idle_response_patch_dict_20260630(data, reply)
                    encoded = _nova_project_state_idle_json_final_20260630.dumps(data, ensure_ascii=False)
        ...
        def _nova_project_state_idle_response_get_content_20260630(result):
            if isinstance(result, dict):
                return _nova_project_state_idle_response_content_from_dict_20260630(result)
            ...
            if hasattr(result, "get_data"):
                try:
                    ...
                    data = _nova_project_state_idle_json_final_20260630.loads(raw)
                    if isinstance(data, dict):
                        return _nova_project_state_idle_response_content_from_dict_20260630(data)
                    return raw
                except Exception:
                    try:
        ...
        def _nova_project_state_idle_next_response_handle_20260630(self, *args, **kwargs):
            result = _NOVA_PRE_PROJECT_STATE_IDLE_NEXT_RESPONSE_HANDLE_20260630(self, *args, **kwargs)

            try:
                user_text = _nova_project_state_idle_response_extract_text_20260630(args, kwargs).strip().lower()
                content = _nova_project_state_idle_response_get_content_20260630(result)

                is_idle_next = user_text in {"next", "k", "ok", "okay", "continue"}
                is_idle_execution_fallback = "no active execution mission" in str(content or "").lower()

                if is_idle_next and is_idle_execution_fallback:
                    reply = _nova_project_state_idle_response_load_answer_20260630(user_text)
                    if reply:
                        return _nova_project_state_idle_response_patch_result_20260630(result, reply)
            except Exception as _nova_project_state_idle_next_response_error_20260630:
                try:
                    print(
                        "[NOVA_PROJECT_STATE_IDLE_NEXT_RESPONSE_REPAIR_FINAL_20260630] bypass:",
                        _nova_project_state_idle_next_response_error_20260630,
                    )
                except Exception:
                    pass
    ...
    try:
        print(
            "[NOVA_PROJECT_STATE_IDLE_NEXT_RESPONSE_REPAIR_FINAL_20260630] failed:",
            _nova_project_state_idle_next_response_install_error_20260630,
        )
    except Exception:
        pass
...
def _nova_token_usage_extract_assistant_text_20260705(value):
    try:
        ...
        if isinstance(value, dict):
            for key in ("text", "content", "message", "response", "assistant_text"):
                found = value.get(key)
            ...
            if isinstance(nested, dict):
                return _nova_token_usage_extract_assistant_text_20260705(nested)
...
def _nova_token_usage_extract_result_text_20260705(result):
    try:
        if isinstance(result, dict):
            for key in ("assistant_text", "response", "message", "content"):
            ...
            if isinstance(assistant, dict):
                text = _nova_token_usage_extract_assistant_text_20260705(assistant)
                if text.strip():
                    return text
...
def _nova_token_usage_wrap_finalize_response_20260705():
    try:
        cls = ChatService
    except Exception:
        return

    if getattr(cls, "_nova_token_usage_finalize_wrapped_20260705", False):
        return

    original = getattr(cls, "_finalize_response", None)
    if not callable(original):
        return
...
_nova_token_usage_wrap_finalize_response_20260705()

# NOVA_ATTACHMENT_GUARD_WEB_SUPPRESSION_20260705
def _nova_attachment_guard_find_payload_from_args(args=None, kwargs=None):
    args = args or ()
    kwargs = kwargs or {}
    ...
    for value in candidates:
        try:
            from nova_backend.services.chat_attachment_intent_guard import has_attachments

            if has_attachments(value):
                return value
        except Exception:
            continue
    ...
    try:
        ...
        if has_request_context():
            ...
            if boundary_attachments:
                return boundary_attachments
    except Exception:
        pass
...
def _nova_attachment_guard_find_user_text_from_args(args=None, kwargs=None):
    args = args or ()
    kwargs = kwargs or {}
...
def _nova_attachment_guard_web_disabled_result(user_text=""):
    return {
        "ok": True,
        "disabled": True,
        "suppressed": True,
...
def _nova_install_attachment_guard_web_suppression():
    ...
    if getattr(target_class, "_nova_attachment_guard_web_suppression_installed", False):
        return True

    original = getattr(target_class, "_execute_web_fetch", None)

    if original is None:
        return False

    def _nova_guarded_execute_web_fetch(self, *args, **kwargs):
        user_text = _nova_attachment_guard_find_user_text_from_args(args, kwargs)
        payload = _nova_attachment_guard_find_payload_from_args(args, kwargs)
        ...
        try:
            ...
            if should_suppress_web_for_attachment(user_text, payload):
                return _nova_attachment_guard_web_disabled_result(user_text)
        except Exception:
            pass

        return original(self, *args, **kwargs)

    setattr(target_class, "_nova_original_execute_web_fetch", original)
    setattr(target_class, "_execute_web_fetch", _nova_guarded_execute_web_fetch)
    setattr(target_class, "_nova_attachment_guard_web_suppression_installed", True)
