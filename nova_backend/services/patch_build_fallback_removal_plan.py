...
REQUIRED_SMOKES = [
    ...
    "python .\\tools\\nova_patch_build_adapter_smoke.py",
...
FORBIDDEN_ACTIONS = [
    ...
    "Do not touch autonomy-plan adapter behavior.",
...
def create_patch_build_fallback_removal_plan(app_path: str = "app.py") -> Dict[str, object]:
    path = Path(app_path)
    ...
    return {
        ...
        "removal_scope": [
            "Only the old patch-build fallback block is eligible for future removal.",
            "The future removal block starts at NOVA_PATCH_BUILD_COMMAND_GUARD_20260630.",
            "The future removal block includes _nova_extract_patch_build_goal_20260630.",
            "The future removal block includes nova_patch_build_command_guard_20260630.",
            "The future removal must not remove NOVA_PATCH_BUILD_ADAPTER_GUARD_20260701.",
        ],
        "required_smokes": REQUIRED_SMOKES,
        "forbidden_actions": FORBIDDEN_ACTIONS,
        "next_step": "Lock this plan first. If approved later, remove only the patch-build fallback block and run the required smokes before commit.",
    }


def format_patch_build_fallback_removal_plan(app_path: str = "app.py") -> str:
    plan = create_patch_build_fallback_removal_plan(app_path)
    target = plan["target"]
...
if __name__ == "__main__":
    ...
    print(format_patch_build_fallback_removal_plan(input_path))
