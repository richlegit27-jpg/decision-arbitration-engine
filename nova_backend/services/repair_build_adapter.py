...
def extract_repair_build_input(user_text: str) -> Optional[str]:
    raw = str(user_text or "").strip()
    lowered = raw.lower()

    for prefix in REPAIR_BUILD_PREFIXES:
...
def build_repair_build_response(
    payload: Dict[str, Any],
    session_service: Any = None,
) -> Optional[Dict[str, Any]]:
    data = payload if isinstance(payload, dict) else {}
    user_text = _payload_user_text(data)
    failed_output = extract_repair_build_input(user_text)
