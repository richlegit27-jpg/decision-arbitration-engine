...
from nova_backend.tools.tool_runtime_service import tool_runtime_service
...
class ChatService:
    ...
    def __init__(
        ...
        # =====================================================
        # REAL NOVA TOOL RUNTIME
        # =====================================================
        self.nova_tool_registry = load_tools()

        self.nova_tool_executor = (
            tool_executor
            or ToolExecutor(
                nova_tool_registry=self.nova_tool_registry,
            )
        )
        ...
        self.execution_step_service = ExecutionStepService(
            ...
            tool_executor=self.nova_tool_executor,
        )
...
"""
    ...
    def _execute_current_step(
        ...
        try:
            ...
            tool_bundle = {
                "ok": False,
                "error": "tool_execution_bridge_failed",
                "details": repr(exc),
            }

        tool_context = ""

        if isinstance(tool_bundle, dict) and tool_bundle:
            ...
            if isinstance(tool_result, dict):
                ...
                if tool_result.get("ok") is False:
                    ...
                    if tool_error:
                        ...
                        tool_context = (
                            "Live tool execution failed.\n"
                            f"Tool: {self.safe_str(tool_decision.get('tool_name'))}\n"
                            f"Error: {tool_error}"
                        )
                    ...
                    if tool_result.get("content"):
                        ...
                        tool_text = self.safe_str(
                            tool_result.get("content")
                        )

                    elif tool_result.get("matches"):

                        tool_text = str(
                            tool_result.get("matches")
                        )
                    ...
                    elif tool_result.get("items"):

                        tool_text = str(
                            tool_result.get("items")
                        )
        ...
        try:
            ...
            step_output = self._extract_text_response(
                response
            ).strip()
    ...
    def _guess_search_query_from_text(self, text: str) -> str:
        ...
        patterns = [
            r"(?:search for|search|find|locate)\s+(.+?)(?:\s+in\s+[A-Za-z]:\\|$)",
            r"(?:search for|search|find|locate)\s+(.+)",
        ]
    ...
    def _decide_tool_for_step(
        ...
        working_state = (
            execution.get("working_state")
            or {}
        )

        path = (
            self._guess_path_from_text(
                user_text
            )
            or self._guess_path_from_text(
                self.safe_str(
                    execution.get("goal")
                )
            )
        ...
        if any(
            ...
            return {
                ...
                "args": args,
                "reason": (
                    "port inspection request"
                ),
            }

        # =============================
        # DIRECTORY / PROJECT STRUCTURE
        # Must come before generic inspect/read.
        # =============================

        is_directory_step = any(
            phrase in step_title
            for phrase in [
                "list files",
                "directory",
                "folder",
        ...
        if is_search_step:
            ...
            query = (
                self._guess_search_query_from_text(
                    user_text
                )
            )
            ...
            if query:
                ...
                if (
                    search_path
                    and os.path.isfile(search_path)
                ):
        ...
        try:
            ...
            if tool_name == "auto_fix_file":
                ...
                return {
                    ...
                    "user_text": user_text,
                    "result": (
                        "Auto-fix target detected:\n"
                        f"{path}\n\n"
                        "Next: generate pending fix "
                        "from this file and bug details."
                    ),
                }
            ...
            if tool_name == "apply_pending_fix":
                ...
                if self.nova_tool_registry is None:
                    ...
                    return {
                        ...
                        "status": "failed",
                        "error": (
                            "nova_tool_registry_unavailable"
                        ),
                    }
                ...
                if tool is None:
                    ...
                    return {
                        ...
                        "status": "failed",
                        "error": (
                            "apply_pending_fix_tool_not_registered"
                        ),
                    }
            ...
            if self.tool_registry is None:
                ...
                return {
                    ...
                    "status": "failed",
                    "error": (
                        "unified_tool_registry_unavailable"
                    ),
                }
            ...
            return {
                ...
                "status": "failed",
                "error": (
                    "tool_decision_execution_failed"
                ),
                "details": repr(exc),
            }
    def approve_pending_tool(
        ...
        if self.tool_executor is None:
            ...
            return {
                "ok": False,
                "status": "tool_executor_unavailable",
                "error": (
                    "Unified tool executor is not available."
                ),
                "tool": tool_name,
            }
