    ...
    return `
      <div class="chat-list-row ${isActive ? "is-active" : ""}" data-chat-id="${escapeHtml(chatId)}">
        ...
        <div class="chat-list-actions">
          <button type="button" class="chat-list-action" data-action="rename" data-chat-id="${escapeHtml(chatId)}" aria-label="Rename chat">âœŽ</button>
          <button type="button" class="chat-list-action" data-action="delete" data-chat-id="${escapeHtml(chatId)}" aria-label="Delete chat">ðŸ—‘</button>
        </div>
      </div>
    `
  }).join("")
...
async function switchChat(chatId){
  ...
  await chatStorage.loadMessages(nextId)
  renderChatList()
...
async function deleteChat(chatId){
  ...
  const chats = ensureChatsArray()
  if(String(state.activeChatId || "") === targetId){
    state.activeChatId = getChatId(chats[0]) || ""
  }

  if(state.activeChatId){
    await chatStorage.loadMessages(state.activeChatId)
  }else{
    state.messages = []
  }
