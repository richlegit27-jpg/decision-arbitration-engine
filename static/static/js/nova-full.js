...
if(!chatStateApi){
  throw new Error("Nova frontend requires window.NovaChatState");
}
...
async function openNewChat(){
  ...
  try{
    if(!chatStorageApi || !chatStorageApi.createAndSaveChat){
    ...
    if(el.newChatBtn){ el.newChatBtn.disabled = false; el.newChatBtn.textContent = "+ New Chat"; }
  }
}

function setActiveChat(chatId){
  chatStateApi.setActiveChat(chatId);
  renderChatList();
}
...
function deleteChat(chatId){
  if(chatStorageApi && typeof chatStorageApi.deleteChat==="function"){
    chatStorageApi.deleteChat(chatId).then(renderChatList).catch(console.error);
  } else {
    chatStateApi.deleteChat(chatId);
    renderChatList();
  }
}
...
function addAttachment(file){
  if(!chatStateApi.state.pendingAttachments) chatStateApi.state.pendingAttachments = [];
