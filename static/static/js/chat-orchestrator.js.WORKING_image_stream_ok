...
function createChatOrchestrator(options = {}){
  ...
  function hydrateChatTitle(chatId, messages = []){
    ...
    if(typeof chatStorage.hydrateChatFromMessageActivity === "function"){
      const firstUserText = (Array.isArray(messages) ? messages : []).find(message => {
        return String(message?.role || "").toLowerCase() === "user" && String(message?.content || "").trim()
      })

      chatStorage.hydrateChatFromMessageActivity(
        targetChatId,
        String(firstUserText?.content || "").trim()
      )
      return
    }
  ...
  function bindAttachmentCallbacks(){
    ...
    if(typeof attachmentsService.setOnChange === "function"){
      attachmentsService.setOnChange((_files) => {
        composer.updateComposerState()
      })
    }
