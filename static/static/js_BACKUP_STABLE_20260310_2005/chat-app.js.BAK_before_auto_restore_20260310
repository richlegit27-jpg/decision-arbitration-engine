...
if(!chatStorage){
  throw new Error("NovaChatApp: window.NovaChatStorage is required")
}

if(!chatSidebar){
  throw new Error("NovaChatApp: window.NovaChatSidebar is required")
}

if(!chatMessages){
  throw new Error("NovaChatApp: window.NovaChatMessages is required")
}

if(!composer){
  throw new Error("NovaChatApp: window.NovaComposer is required")
}
...
function bindStreamCallbacks(){
  ...
  streamService.setCallbacks({
    ...
    onDone(payload = {}){
      ...
      if(messageId && payload?.content && !chatMessages.findMessageById(messageId)?.content){
    ...
    onError(error){
      console.error("NovaChatApp stream error:", error)
      state.isStreaming = false
      state.streamingMessageId = null
      chatMessages.finishStreamingMessage()
      renderAll()
...
if(document.readyState === "loading"){
  document.addEventListener("DOMContentLoaded", () => {
    init().catch(error => {
      console.error("NovaChatApp init error:", error)
    })
  })
}else{
  init().catch(error => {
