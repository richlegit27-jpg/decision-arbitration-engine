...
function loadOptionalScript(src){
  return new Promise((resolve) => {
    ...
    script.addEventListener("error", () => {
      console.warn(`NovaChatApp optional script failed to load: ${src}`)
      resolve()
    }, { once: true })
...
async function initOptionalFeatures(){
  await loadOptionalScript("/static/js/image-lightbox.js")
  await loadOptionalScript("/static/js/voice-input.js")

  if(window.NovaImageLightbox?.init){
    window.NovaImageLightbox.init()
  }

  if(window.NovaVoiceInput?.init){
