...
function createChatEventsService(options = {}){
  ...
  function handleSidebarToggle(event){
    ...
    if(typeof ui.toggleSidebar === "function"){
      ui.toggleSidebar()
      return
    }
  ...
  function wireDomEvents(){
    ...
    el.btnSidebarToggle?.addEventListener("click", handleSidebarToggle)
    ...
    el.emptySuggestions?.addEventListener("click", async (event) => {
      const button = event.target.closest("[data-suggestion], [data-prompt]")
      if(!button || !el.input){
        return
      }

      el.input.value = String(
