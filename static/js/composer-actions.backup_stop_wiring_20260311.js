...
function createComposerActions(options = {}){
  ...
  function updateComposerState(){
    ...
    const hasText = !!text
    const hasPending = getPendingAttachments().length > 0
  ...
  function clearPendingInputAndAttachments(){
    ...
    clearPendingState()
    attachmentsController?.setPendingFiles?.([])
    attachmentsController?.renderPendingAttachments?.()
    updateComposerState()
  }
  ...
  function isStreamNotAvailableError(error){
    const message = String(error?.message || "")
    return (
      /\/api\/chat\/stream failed \(404\)/i.test(message) ||
      /\/api\/chat_stream failed \(404\)/i.test(message) ||
      /404/i.test(message)
    )
  }
  ...
  function handleNonStreamingReply(reply, fallbackChatId){
    ...
    if(String(responseText || "").trim()){
      appendAssistantMessage(String(responseText))
    }else{
      persistCurrentChatMessages()
    }
  }

  async function sendViaNonStreaming(chatId, text, attachmentPayload){
  ...
  async function sendCurrentMessage(options = {}){
    ...
    const pending = attachmentsController?.ensurePendingAttachments?.() || getPendingAttachments()
    ...
    try{
      ...
      const normalizedUploadedAttachments = Array.isArray(uploadedAttachments)
        ? uploadedAttachments.slice()
        : []

      const attachmentPayload = attachmentsController.makeAttachmentPayload(normalizedUploadedAttachments)

      if(!skipLocalUserAppend){
        appendLocalUserMessage(text, normalizedUploadedAttachments)
      }
      ...
      if(streamService && typeof streamService.send === "function"){
        try{
          ...
          aborted = isAbortLikeError(streamError)

          if(isStreamNotAvailableError(streamError)){
            console.warn("NovaComposer: stream endpoint missing, falling back to /api/chat")
            reply = await sendViaNonStreaming(chatId, text, attachmentPayload)
          }else if(aborted){
            persistCurrentChatMessages()
            return {
              ok: false,
