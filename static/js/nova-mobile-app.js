...
/* -------------------------------------------------
   NOVA MOBILE TEXT POLISH RESTORE FINAL
   Restores markdown/code/bullet polish without touching sessions.
   Preserves generated images and Copy/Regen action rows.
   20260629
-------------------------------------------------- */
(() => {
    if (window.__NOVA_MOBILE_TEXT_POLISH_RESTORE_FINAL_20260629__) return;
    window.__NOVA_MOBILE_TEXT_POLISH_RESTORE_FINAL_20260629__ = true;

    const PRESERVE_SELECTORS = [
        ".nova-generated-image-result",
    ...
    function cleanActionLeak(value) {
        return String(value || "")
            .replace(/(?:\s*Copy\s*Regen\s*)+$/gi, "")
            .replace(/(?:\s*Copy\s*Regenerate\s*)+$/gi, "")
            .replace(/(?:\s*Copied\s*Regen\s*)+$/gi, "")
    ...
    function rawMessageText(el) {
        const clone = el.cloneNode(true);

        clone.querySelectorAll(PRESERVE_SELECTORS).forEach((node) => node.remove());

        return cleanActionLeak(String(clone.innerText || clone.textContent || "").trim());
    }
    ...
    function renderMarkdownLite(text) {
        ...
        while ((match = fenced.exec(raw)) !== null) {
            if (match.index > index) {
                blocks.push({
                    type: "text",
                    value: raw.slice(index, match.index)
                });
            }
        ...
        return blocks.map((block) => {
            ...
            lines.forEach((line) => {
                ...
                if (bullet) {
                    if (!inList) {
                        html.push('<ul class="nova-mobile-bullet-list">');
                        inList = true;
                    }
                ...
                closeList();
                html.push('<p class="nova-mobile-polished-paragraph">' + inlineMarkdown(trimmed) + "</p>");
            });
    ...
    function inlineMarkdown(value) {
        ...
        safe = safe.replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>");
        safe = safe.replace(/`([^`]+)`/g, '<code class="nova-mobile-inline-code">$1</code>');

        return safe;
    }

    function preserveNodes(el) {
        return Array.from(el.children || []).filter((child) => {
            return child.matches && child.matches(PRESERVE_SELECTORS);
        });
    }

    function polishOne(el) {
        if (!isAssistantBubble(el)) return false;
        if (el.dataset.novaTextPolishRestored === "1") return false;

        const raw = rawMessageText(el);

        if (!raw) return false;

        const preserved = preserveNodes(el);

        el.innerHTML = "";

        const content = document.createElement("div");
        content.className = "nova-mobile-polished-content";
        content.innerHTML = renderMarkdownLite(raw);
        ...
        el.classList.add("nova-mobile-polished-bubble");
        el.dataset.novaTextPolishRestored = "1";
    ...
    function polishAll() {
        ...
        Array.from(chat.children || []).forEach((el) => {
            if (polishOne(el)) count += 1;
        });

        if (count) {
            console.log("[Nova Mobile Text Polish Restore] polished", count);

            try {
                if (typeof window.NovaMobileNormalizeStableActions === "function") {
                    window.NovaMobileNormalizeStableActions();
                }
            } catch (e) {}

            try {
                if (typeof window.NovaMobileRenderGeneratedImages === "function") {
                    window.NovaMobileRenderGeneratedImages();
                }
            } catch (e) {}
        }
    ...
    function schedule() {
        clearTimeout(window.__novaMobileTextPolishRestoreTimer);
        window.__novaMobileTextPolishRestoreTimer = setTimeout(polishAll, 160);
    }
    ...
    if (chat && window.MutationObserver) {
        ...
        window.__NovaMobileTextPolishRestoreObserver = observer;
    }

    window.NovaMobileRestoreTextPolish = polishAll;

    console.log("[Nova Mobile Text Polish Restore] ready");
})();
