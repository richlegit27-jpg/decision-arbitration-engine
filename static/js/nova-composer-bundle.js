  ...
  function dedupeMessages(messages) {
    ...
    return safeArray(messages)
      ...
      .filter(function (msg) {
        const key = [
          String(msg.id || "").trim(),
          String(msg.role || "").trim(),
          normalizeText(msg.text || "").trim(),
        ].join("::");
...
function dedupeMessages(messages) {
  ...
  return (messages || [])
    ...
    .filter(function (msg) {
      ...
      if (stableId && !seen.has("id::" + stableId)) {
        seen.add("id::" + stableId);
      }
...
function autoResizeTextarea() {
  ...
  els.chatInput.style.height =
    Math.min(els.chatInput.scrollHeight, 280) + "px";
}
...
function upsertMessage(rawMessage) {
  ...
  const existingIndex = state.messages.findIndex(function (item) {
    ...
    if (
      ...
      return (
        itemText.slice(0, 180) === msgText.slice(0, 180)
      );
    }

    return itemText === msgText;
  });
...
async function sendMessage() {
  if (state.isSendingMessage) {
    console.warn("sendMessage blocked: already sending");
    return;
  }
...
function renderWeb() {
  ...
  if (
    ...
    state.rail.selectedKind === "web" &&
    state.rail.selectedId &&
    typeof setRailSelectedItem === "function"
  ) {
    const selectedStillExists = !!document.querySelector(
      '[data-web-id="' + cssEscape(state.rail.selectedId) + '"]'
    );

    if (selectedStillExists) {
      setRailSelectedItem("web", state.rail.selectedId);
    } else {
      state.rail.selectedKind = "";
      state.rail.selectedId = "";
    }
...
if (
  ...
  // fallback assistant insertion disabled
  // finalizeStreamMessage now owns final assistant rendering
}
