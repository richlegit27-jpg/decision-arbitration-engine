  ...
  els.chatMessages.innerHTML = groups.map(function (group) {
    ...
    const parts = group.items.map(function (entry) {
      ...
      const isStreamingPart =
        role === "assistant" &&
        entry === group.items[group.items.length - 1] &&
        state.streamingActive;

      return '<div class="nova-message-part"><div class="' +
        (isStreamingPart ? 'nova-message-part-body is-streaming' : 'nova-message-part-body') +
        '">' + txt + '</div></div>';
    }).join("");

    return [
      '<div class="nova-message ' + escapeHtml(role) + (role === "assistant" && state.streamingActive ? ' is-streaming' : '') + '">',
