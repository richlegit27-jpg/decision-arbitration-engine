...
function handleStreamEvent(event) {
  ...
  const stream = ensureStreamState();
  ...
  // create placeholder immediately (fixes delay + snap)
  if (!stream.targetMessageId) {
    ...
    startStreamPlaceholder(id);
  }

  // ignore start noise
  if (type === "start" || type === "message_start" || type === "response_start") {
    return;
  }
  ...
  if (type === "error") {
    finalizeStreamMessage({
      error: true,
      text: payload.error || payload.message || "Stream failed."
    });
    return;
  }
  ...
  // fallback text (important for mixed responses)
...
try {
  ...
  typeof state.stream.controller.signal !== "undefined"
    ? state.stream.controller.signal
    : undefined,
    });
