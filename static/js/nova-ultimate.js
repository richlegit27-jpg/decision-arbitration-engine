(() => {
  ...
  function applyLayout() {
    ...
    if (toggleSidebarBtn) {
      ...
      toggleSidebarBtn.title = app.state.sidebarOpen ? "Collapse sidebar" : "Expand sidebar";
    }
  ...
  function renderMessages(messages) {
    ...
    for (const msg of list) renderMessage(msg);
    updateChatHeader();
  }
  ...
  async function sendMessage() {
    ...
    try {
      ...
      let assistantText = "";
      let finished = false;
      let activeStreamBodyEl = getChatContainer()?.querySelector(".message.streaming .message-body") || streamBodyEl;
      ...
      while (true) {
        ...
        while ((splitIndex = buffer.indexOf("\n\n")) !== -1) {
          ...
          if (eventName === "start") {
            ...
            assistantText += delta;
            if (activeStreamBodyEl) {
              activeStreamBodyEl.innerHTML = escapeHtml(assistantText).replace(/\n/g, "<br>");
            }
