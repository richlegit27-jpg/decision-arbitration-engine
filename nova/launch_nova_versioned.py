...
def main():
    print("ðŸ” Running versioned preflight check for Nova...")
    ...
    if outdated_js:
        print(f"âš ï¸ Outdated JS files: {outdated_js}")
    ...
    if outdated_css:
        print(f"âš ï¸ Outdated CSS files: {outdated_css}")

    if any_missing:
        print("\nâš ï¸ Fix missing/outdated files before launching Nova!")
        sys.exit(1)
    else:
        print("âœ… All files present and up-to-date. Launching Nova...")
