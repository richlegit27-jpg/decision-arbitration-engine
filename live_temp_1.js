...
function getSessionTitle(session) {
  ...
  return session.name
    ...
    window.loadDesktopMemoryLegacy = async function loadDesktopMemoryLegacy() {
...
async function loadDesktopSessions() {
  ...
  window.__NOVA_SESSION_V2_LOCK__ = true;
  ...
  try {
    ...
    sessions.forEach(session => {
      console.log("RENDER SESSION OBJECT", session);

      const sid = session.id;

      const btn = document.createElement("div");
      btn.className = "nova-v2-session-item";
      ...
      btn.onclick = async () => {
        ...
        try {
          if (typeof window.NovaDesktopFetchSession === "function") {
            ...
            if (
              typeof renderDesktopChatMessagesRescue === "function" &&
...
window.NovaDesktopLoadSessions = loadDesktopSessions;

// NOVA_INITIAL_SESSION_LOAD_V2

function novaInitialSessionLoadV2() {
    if (typeof window.loadDesktopSessions === "function") {
        window.loadDesktopSessions();
        console.log("[Nova] v2 session loader started");
    } else {
        console.warn("[Nova] session loader not ready, retrying");
        setTimeout(novaInitialSessionLoadV2, 250);
    }
}
