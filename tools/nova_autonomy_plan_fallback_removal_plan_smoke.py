...
from nova_backend.services.autonomy_plan_fallback_removal_plan import (
    create_autonomy_plan_fallback_removal_plan,
    format_autonomy_plan_fallback_removal_plan,
)
...
def main():
    plan = create_autonomy_plan_fallback_removal_plan(str(APP_PATH))
    text = format_autonomy_plan_fallback_removal_plan(str(APP_PATH))

    assert_true("plan mode", plan.get("mode") == "autonomy_plan_fallback_removal_plan_only")
    ...
    assert_contains(
        "autonomy-plan fallback removal plan text",
        text,
        [
    ...
    print("NOVA AUTONOMY PLAN FALLBACK REMOVAL PLAN SMOKE PASSED")
    return 0
