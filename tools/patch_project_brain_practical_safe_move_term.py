...
new = '''        f"Next concrete safe move: {context.next_move} "
        "Safe validation: run the context-builder smoke, project-state memory API smoke, general-intelligence smoke, "
'''

if old not in text:
    raise SystemExit("target next concrete move text not found")

text = text.replace(old, new, 1)
path.write_text(text, encoding="utf-8")
print("patched practical Project Brain answer to include exact safe move term")
