...
def ask(message):
    payload = {
        "message": message,
        "session_id": (
            f"failure_interpreter_api_smoke_{int(time.time())}"
        ),
        "attachments": [],
    }
    ...
    try:
        ...
        raise RuntimeError(
            f"Request failed for {message!r}: {exc}"
        ) from exc
...
def check_failure(
    ...
    print("")
    print(
        f"QUESTION: {question.splitlines()[0]}"
    )
    ...
    assert_true(
        ...
        "project brain mission control" in lower,
        answer,
    )

    assert_true(
        "mission control intent",
        "intent: mission_control" in lower,
        answer,
    )

    assert_true(
        "failure type field",
        "failure type:" in lower,
        answer,
    )

    assert_true(
        "failure severity field",
        "failure severity:" in lower,
        answer,
    )

    assert_true(
        "failure patch target field",
        "failure patch target:" in lower,
        answer,
    )

    assert_true(
        "failure next command field",
        "failure next command:" in lower,
        answer,
    )

    assert_true(
        "expected failure type",
        f"failure type: {expected_type}" in lower,
        answer,
    )
...
def check_direct_recall_not_hijacked():
    ...
    assert_true(
        "no failure fields",
        (
            "failure type:" not in lower
            and "failure next command:" not in lower
        ),
        answer,
    )
...
def main():
    print(
        "NOVA PROJECT BRAIN FAILURE INTERPRETER API SMOKE"
    )
    print(
        "================================================"
    )
    ...
    print("")
    print(
        "NOVA PROJECT BRAIN FAILURE INTERPRETER API SMOKE PASSED"
    )
