...
def main():
    ...
    assert_true("planner recommends conversation quality after completed filter", plan.recommended_move == "Nova Conversation Quality Field Test v1", plan)
    assert_true("planner risk low", plan.risk == "low", plan)
    assert_true("planner has target files", "nova_backend/services/project_brain_upgrade_radar.py" in plan.target_files, plan)
    assert_true("planner has focused smoke", "python .\\tools\\nova_regression_smoke.py" in plan.focused_smokes, plan)
    ...
    plan.ranked_moves[0]["name"] == "Nova Conversation Quality Field Test v1",
    plan.ranked_moves
)
    ...
    assert_true("cleanup strategy recommended", cleanup_plan.recommended_move == "Nova Conversation Quality Field Test v1", cleanup_plan)
    assert_true("cleanup low risk", cleanup_plan.risk == "low", cleanup_plan)
    ...
    assert_true("failure interpreter recommended", failure_plan.recommended_move == "Nova Conversation Quality Field Test v1", failure_plan)
    ...
    assert_true("dict has recommended move", data.get("recommended_move") == "Nova Conversation Quality Field Test v1", data)
    ...
    assert_true("card recommended move", "Recommended move: Nova Conversation Quality Field Test v1" in card, card)
